"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TypesEnvironmentService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const types_environment_entity_1 = require("./entities/types-environment.entity");
let TypesEnvironmentService = class TypesEnvironmentService {
    constructor(typesEnvironmentRepository) {
        this.typesEnvironmentRepository = typesEnvironmentRepository;
    }
    async create(createTypesEnvironmentDto) {
        const typesEnvironment = this.typesEnvironmentRepository.create(createTypesEnvironmentDto);
        return this.typesEnvironmentRepository.save(typesEnvironment);
    }
    findAll() {
        return this.typesEnvironmentRepository.find();
    }
    findOne(id) {
        return this.typesEnvironmentRepository.findOne({ where: { id: id } });
    }
    findOneName(name) {
        console.log("Nombre:", name);
        return this.typesEnvironmentRepository.findOne({ where: { name: name } });
    }
    async update(id, updateTypesEnvironmentDto) {
        await this.typesEnvironmentRepository.update(id, updateTypesEnvironmentDto);
        return this.typesEnvironmentRepository.findOne({ where: { id } });
    }
    async remove(id) {
        await this.typesEnvironmentRepository.delete(id);
    }
};
exports.TypesEnvironmentService = TypesEnvironmentService;
exports.TypesEnvironmentService = TypesEnvironmentService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(types_environment_entity_1.TypesEnvironment)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TypesEnvironmentService);
//# sourceMappingURL=types-environments.service.js.map