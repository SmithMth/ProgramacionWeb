...
function ReservasUser() {
  ...
  const fetchEnvironments = async () => {
    try {
      const environments = await getAllEnvironmentsWithDetails();
  ...
  const filteredEnvironments = environments.filter((environment) => {
    ...
    const tipoF =
      tipos.length === 0 || tipos.some((tipo) => environment.typeEnvironment?.name === tipo.name);
    const facilidadesF =
      facilidades.length === 0 ||
      facilidades.every((facilidad) => environment.facilities.some((f) => f.name === facilidad.name));
    const aceptadoF = !aceptado || environment.aceptado === aceptado;
    const vigenteF = !vigente || environment.vigente === vigente;

    if (!filtro) return true;
    return capacidadF && enabledF && tipoF && facilidadesF && aceptadoF && vigenteF;
  });
    ...
    <>
      <div className="p-6 sm:p-5 bg-white rounded-lg shadow-xl w-11/12">
        ...
        <div className="space-y-4">
          <Swich name='Aceptado' value={aceptado} setValue={setAceptado}></Swich>
          <Swich name='Vigente' value={vigente} setValue={setVigente}></Swich>
